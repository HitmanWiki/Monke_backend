/**
 * MONKE BET AI Match Analysis Agent — Google Search Grounded
 * ─────────────────────────────────────────────────────────────
...
class AIMatchAgent {
  ...
  async predict(homeTeam, awayTeam, options = {}) {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(`🐒 MONKE BET AGENT: ${homeTeam}  vs  ${awayTeam}`);
    console.log(`🏆 Competition: ${competition}`);
    console.log(`🔍 Gemini will search Google for live data...`);
    console.log(`⚡ Powered by Solana`);
    ...
    parsed._meta = {
      ...
      geminiCalls  : 1,
      searchUsed   : true,
      platform     : "MONKE BET on Solana",
      sources      : groundingMetadata?.groundingChunks
        ?.map(c => ({ title: c.web?.title, url: c.web?.uri }))
        .filter(s => s.url) || [],
    };

    console.log(`\n✅ MONKE BET analysis complete — sources read: ${parsed._meta.sources.length}`);
    return parsed;
  }
...
You are an elite football analyst and sports betting specialist for MONKE BET — a decentralized betting platform on Solana blockchain.
...
After searching and reading the results, produce a full betting analysis for MONKE BET users on Solana.
...
- Remember MONKE BET runs on Solana — fast, low-cost transactions`;
  }
...
async function main() {
  ...
  console.log("\n🌍 Testing World Cup match (neutral venue) for MONKE BET...");
  const result1 = await agent.predict("South Africa", "Korea Republic", {
    competition : "FIFA World Cup 2026",
    verbose     : true,
  });

  console.log("\n====== MONKE BET WORLD CUP PREDICTION ======");
  ...
  console.log("\n====== MONKE BET PREMIER LEAGUE PREDICTION ======");
