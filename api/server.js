/**
 * MONKE BET — World Cup 2026 Betting Backend API
...
const { Connection, Keypair, Transaction, sendAndConfirmTransaction, BpfLoader, BPF_LOADER_PROGRAM_ID, PublicKey } = require('@solana/web3.js');
const fs = require('fs');
...
async function fetchAndCacheMatches() {
  console.log("📅 Loading World Cup 2026 fixtures for MONKE BET...");
  ...
  console.log(`✅ Stored ${stored}/104 World Cup matches in DB for MONKE BET`);
  return stored;
}
...
console.log("🐒 MONKE BET AI Agent ready");
...
app.get("/", (req, res) => res.json({
  name: "MONKE BET World Cup 2026 API",
  description: "Official 104-match schedule with live result fetching on Solana",
  endpoints: {
...
async function start() {
  console.log("\n" + "=".repeat(55));
  console.log("🐒 MONKE BET — World Cup 2026 API on Solana");
